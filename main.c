...
#include <limits.h>
#include <getopt.h>
#include <dirent.h> // <-- NEW HEADER for directory operations
...
// --- `do_run` function with one small addition ---
int do_run(int argc, char *argv[]) {
    // ... all the setup logic and getopt_long loop are identical ...
    ...
    // --- NEW ADDITION: Save the command to the state directory ---
    char cmd_path[PATH_MAX];
    snprintf(cmd_path, sizeof(cmd_path), "%s/command", state_dir);
    FILE *cmd_file = fopen(cmd_path, "w");
    if (cmd_file) {
        for (int i = 1; container_argv[i] != NULL; i++) {
            fprintf(cmd_file, "%s ", container_argv[i]);
        }
        fclose(cmd_file);
    }
    // --- END OF NEW ADDITION ---
    ...
    char procs_path[PATH_MAX]; snprintf(procs_path, sizeof(procs_path), "%s/cgroup.procs", cgroup_path); write_file(procs_path, "self");
...
// Helper function to print a file's content
void print_file_content(const char *label, const char *path) {
    char buf[1024] = {0};
    FILE *f = fopen(path, "r");
    if (!f) return;
    fread(buf, 1, sizeof(buf) - 1, f);
    printf("%-20s: %s", label, buf);
    fclose(f);
}

// --- NEW `do_status` implementation ---
int do_status(int argc, char *argv[]) {
    ...
    print_file_content("CPU Stats", path_buffer);
