...
int do_run(int argc, char *argv[]) {
    ...
    if (detach_flag) {
        // In detach mode, we redirect our own output so we don't get stopped by the shell
    ...
    if (detach_flag) {
        printf("%d\n", container_pid); // Print PID to the (now detached) stdout
        return 0; // Parent exits immediately
    }

    // Foreground container: wait for it to finish and then clean up
    int child_status;
...
int do_list(int argc, char *argv[]) {
    ...
    while ((dir_entry = readdir(d)) != NULL) {
        ...
        if (access(proc_path, F_OK) == 0) {
            ...
            // "Reaper" logic: process is dead, clean up its directories
