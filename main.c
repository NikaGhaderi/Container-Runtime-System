...
void setup_cgroup_hierarchy() {
    ...
    // Use the robust shell command to enable controllers, suppressing errors if it's already set.
...
int container_main(void *arg) {
    ...
    // This part only runs if execv fails
    perror("execv failed");
    return 1;
}
...
int do_rm(int argc, char *argv[]) {
    ...
    if (access(proc_path, F_OK) == 0) {
        fprintf(stderr, "Error: Cannot remove a running container. Stop it first using 'stop %s'.\n", pid_str);
        return 1;
    }
    ...
    char cmd_path[PATH_MAX]; snprintf(cmd_path, sizeof(cmd_path), "%s/command", state_dir);
    // Note: We cannot clean up the overlay layers for a stopped container easily,
    // as we don't save the random ID. This is a known limitation.
    remove(cmd_path); rmdir(state_dir); rmdir(cgroup_dir);
